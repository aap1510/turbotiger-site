(function (root) {
  ...
  IC.Screens["visao-geral"] = function (deps) {
    ...
    function render() {
      ...
      if (!hasHistory) html += UI.banner("Sua Central está pronta", "Ainda não há rodadas analisadas nesta conta. Defina suas regras e consulte os dados da Comunidade enquanto seu histórico é formado.", "neutral") + '<div class="ic-quick-actions">' + UI.button("Minhas regras", { route: "regras-pausas", icon: "shield" }) + (deps.featureEnabled("ic_community_report_enabled") ? UI.button("Conhecer a Comunidade", { route: "comunidade", icon: "community" }) : '') + '</div>';
      var metrics = [
    ...
    function renderHistoricalSummary(summary) {
      ...
      var topGames = games.slice(0, 2).map(function (game) {
        return '<article class="ic-card"><div class="ic-card__header"><div><h3>' + Core.escapeHtml(game.jogo || game.game) + '</h3><p>' + Core.formatNumber(game.rodadas || 0, 0) + ' rodadas · ' + Core.formatNumber(game.sessoes || 0, 0) + ' sessões</p></div>' + UI.badge("até " + Core.formatNumber(game.maior_multiplicador || 0) + "×", "neutral") + '</div><div class="ic-list">' + UI.listRow("Retorno observado", "Histórico deste jogo", Core.formatPercent(game.rtp, 2)) + UI.listRow("Resultado líquido", "No conjunto analisado", Core.formatSignedMoney(String(game.resultado), currency, places)) + UI.listRow("Premiações relevantes", "Quantidade com retorno ≥ 5× / 10× / 20× / 50× / 100×", [game.m5, game.m10, game.m20, game.m50, game.m100].map(function (value) { return Core.formatNumber(value, 0); }).join(" / ")) + UI.listRow("Perdas disfarçadas de ganho", "Retorno maior que zero, mas menor que a aposta", Core.formatNumber(game.perdas_disfarcadas || 0, 0)) + '</div></article>';
      }).join("");
      return '<section><div class="ic-card__header"><div><h3>Seu histórico analisado</h3><p>Um resumo de todas as rodadas disponíveis nesta conta.</p></div>' + UI.badge("exploratório", "neutral") + '</div>' + UI.banner("Histórico pessoal", "Os números descrevem seus registros. Eles não preveem a próxima rodada.", "neutral") + '<div class="ic-summary-grid">' + UI.metric("Rodadas", Core.formatNumber(overall.rodadas || overall.rounds, 0), "Em " + Core.safeText(overall.sessoes || 0) + " sessões") + UI.metric("Resultado das rodadas", Core.formatSignedMoney(result, currency, places), "Não representa saldo atual nem lucro financeiro reconciliado", Number(result) < 0 ? "negative" : "neutral") + UI.metric("Sessão mediana", Core.formatSignedMoney(String(sessions.resultado_mediano || 0), currency, places), "Metade ficou abaixo e metade acima") + UI.metric("Multiplicador máximo", Core.formatNumber(overall.maior_multiplicador || 0) + "×", "Maior coeficiente observado") + UI.metric("Premiações ≥ 100x", Core.formatNumber(overall.m100 || 0, 0), "Eventos extremos") + UI.metric("Perdas disfarçadas", Core.formatNumber(overall.perdas_disfarcadas || 0, 0), "Retorno parcial com perda líquida") + '</div><div class="ic-grid ic-grid--cards">' + topGames + '</div><div class="ic-disclaimer">Retorno total elevado pode depender de poucos prêmios extremos. As telas de Estatísticas e Comunidade mostram amostra, concentração e resultado sem o maior pagamento.</div></section>';
    }
    ...
    function renderRadar(radar) {
      ...
      if (!facts.length) return UI.banner("Radar da sessão", "Sem contexto atual suficiente para avaliar seus limites. Isso não invalida as análises do seu histórico.", "neutral");
