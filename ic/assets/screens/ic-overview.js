  ...
  IC.Screens["visao-geral"] = function (deps) {
    ...
    function renderHistoricalSummary(summary) {
      ...
      var topGames = games.slice(0, 4).map(function (game) {
        return '<article class="ic-card"><div class="ic-card__header"><div><h3>' + Core.escapeHtml(game.jogo || game.game) + '</h3><p>' + Core.safeText(game.rodadas || 0) + ' rodadas · ' + Core.safeText(game.sessoes || 0) + ' sessões</p></div>' + UI.badge("até " + Core.formatNumber(game.maior_multiplicador || 0) + "×", "neutral") + '</div><div class="ic-list">' + UI.listRow("Retorno observado", "Histórico deste jogo", Core.formatPercent(game.rtp, 2)) + UI.listRow("Resultado líquido", "No conjunto analisado", Core.formatSignedMoney(String(game.resultado), currency, places)) + UI.listRow("Premiações relevantes", "5x / 10x / 20x / 50x / 100x", [game.m5, game.m10, game.m20, game.m50, game.m100].map(Core.safeText).join(" / ")) + UI.listRow("Perdas disfarçadas de ganho", "Retorno maior que zero, mas menor que a aposta", Core.safeText(game.perdas_disfarcadas || 0)) + '</div></article>';
      }).join("");
