(function (root) {
  ...
  IC.Screens["regras-pausas"] = function (deps) {
    ...
    function content() {
      ...
      if (!rules.length && catalog().length) rulesHtml = '<p>Escolha um compromisso e defina seu limite. Nada é ativado sem sua confirmação.</p><div class="ic-quick-actions">' + catalog().map(function (item) { return UI.button(item.title, { action: "rule-template", value: item.type, icon: "shield" }); }).join("") + '</div>';
    ...
    async function handleAction(action, value) {
      ...
      if (action === "rule-template" && definition(value)) { state.editing = null; UI.openSheet({ eyebrow: "Regra pessoal", title: "Defina seu compromisso", html: ruleForm(value) }); }
